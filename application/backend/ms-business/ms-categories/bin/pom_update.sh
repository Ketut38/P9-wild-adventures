...
function validatePomExists() {
  ...
  if [ -f pom.xml ] ; then
    echo -e "Found pom.xml file: [\e[32mms-categories\e[0m/pom.xml]"
  else
...
function updateProjectPomsToNextVersion() {
  ...
  echo -e  "Updating \e[32mms-categories\e[0m version from \e[91m[${CURRENT_PROJECT_VERSION}]\e[0m to \e[32m[${projectVersion}]\e[0m...";
